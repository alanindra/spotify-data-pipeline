WITH
    ...
    , monthly_streaks_top_10_stream as (
        ...
        from (
            select 
                ...
                , sumdiff
                , CASE
                    WHEN sumdiff - lag(sumdiff) over (partition by track, artist order by year_month) = 0 then 'STREAKS' 
                    WHEN sumdiff - lag(sumdiff) over (partition by track, artist order by year_month) is null then 'FIRST STREAM' 
                    ELSE 'NO STREAKS'
                END AS streak_status
            from (
                select 
                    *
                    , sum(diff) over (partition by track, artist order by year_month) sumdiff
                from (
                    select 
                        year_month
                        ...
                        , CASE 
                            WHEN (CAST(substr(year_month, 1, 4) AS INT) * 12 + CAST(substr(year_month, 6, 2) AS INT))
                                ...
                                over (partition by track, artist order by year_month) = 1 then 0
                            WHEN (CAST(substr(year_month, 1, 4) AS INT) * 12 + CAST(substr(year_month, 6, 2) AS INT))
                                - lag(CAST(substr(year_month, 1, 4) AS INT) * 12 + CAST(substr(year_month, 6, 2) AS INT))
                                over (partition by track, artist order by year_month) is null then 1
                            ELSE (CAST(substr(year_month, 1, 4) AS INT) * 12 + CAST(substr(year_month, 6, 2) AS INT))
                                - lag(CAST(substr(year_month, 1, 4) AS INT) * 12 + CAST(substr(year_month, 6, 2) AS INT))
                                over (partition by track, artist order by year_month)
                        END AS diff
    ...
    , monthly_streaks_agg AS (
        ...
        FROM (
            SELECT 
                ...
                , streak_status
                , streaks_helper
                , dense_rank() over (partition by track, artist order by streaks_helper) as nth_streak
            FROM (
                SELECT 
                    *
                    , sum(case when streak_status in ('FIRST STREAM','NO STREAKS') then 1 else 0 end)
                        over (partition by track, artist order by year_month) as streaks_helper
    ...
    , daily_streaks_top_10_stream as (
        ...
        from (
            select 
                ...
                , sumdiff
                , CASE
                    WHEN sumdiff - lag(sumdiff) over (partition by track, artist order by date) = 0 then 'STREAKS' 
                    WHEN sumdiff - lag(sumdiff) over (partition by track, artist order by date) is null then 'FIRST STREAM' 
                    ELSE 'NO STREAKS'
                END AS streak_status
            from (
                select 
                    *
                    , sum(diff) over (partition by track, artist order by date) sumdiff
                from (
                    select 
                        date
                        ...
                        , CASE 
                            WHEN julianday(date)
                                - lag(julianday(date))
                                over (partition by track, artist order by date) = 1 then 0
                            WHEN julianday(date)
                                - lag(julianday(date))
                                over (partition by track, artist order by date) is null then 1
                            ELSE julianday(date)
                                - lag(julianday(date))
                                over (partition by track, artist order by date)
                        END AS diff
        ...
        FROM (
            ...
            FROM (
                SELECT 
                    *
                    , sum(case when streak_status in ('FIRST STREAM','NO STREAKS') then 1 else 0 end)
                        over (partition by track, artist order by date) as streaks_helper
    ...
    , final AS (
        select 
            'Monthly' AS period
            , * 
            , row_number() over (partition by track, artist order by cnt_streaks DESC) as streak_rank
